...
class NFreqBinningFunction(BinningFunction):
    ...
    def get_group_labels(self, df: pd.DataFrame, field: str) -> pd.Series[pd.CategoricalDtype]:
        bins = pd.qcut(df[field], q=self.n_freq, duplicates='drop')
        return bins.astype(str).astype('category')
...
class QuartileBinningFunction(BinningFunction):
    ...
    def get_group_labels(self, df: pd.DataFrame, field: str) -> pd.Series[pd.CategoricalDtype]:
        return pd.qcut(df[field], q=4, duplicates='drop').astype(str).astype('category')
...
class DecileBinningFunction(BinningFunction):
    ...
    def get_group_labels(self, df: pd.DataFrame, field: str) -> pd.Series[pd.CategoricalDtype]:
        return pd.qcut(df[field], q=10, duplicates='drop').astype(str).astype('category')
...
class Distro:
    ...
    def _update_graph(self, use_dark_mode, plot_settings, schema):
        ...
        try:
            ...
            ser_x_labels = x_binfunc.bin(df, field=plot_settings.x_column)[x_binfunc.BINLABEL_COLUMN]
            ser_y_labels = y_binfunc.bin(df, field=plot_settings.y_column)[y_binfunc.BINLABEL_COLUMN]
            bin_df = pd.DataFrame({
                'x': ser_x_raw,
                'y': ser_y_raw,
            ...
            for i, category in enumerate(ser_color_group.cat.categories):
                ...
                group_df_raw = df[ser_color_group == category]
                ...
                group_color_value = group_df_raw[color_column] if color_column else pd.Series(dtype='float', index=group_df.index)
                ...
                if plot_settings.dimensionality == '2d':
                    ...
                    fig.add_trace(
                        go.Histogram(
                            x=group_x, nbinsx=x_binfunc.n_histogram_bins, bingroup=1,
                            name=str(category), legendgroup=str(category),
                            marker=dict(
                                opacity=0.5,
                    ...
                    fig.add_trace(
                        go.Histogram(
                            y=group_y, nbinsy=y_binfunc.n_histogram_bins, bingroup=2,
                            name=str(category), legendgroup=str(category),
                            marker=dict(
                                opacity=0.5,
                                color=discrete_colorscale[i] if plot_settings.color_enabled and plot_settings.color_column_type == 'discrete' else discrete_colorscale[0] #type:ignore
